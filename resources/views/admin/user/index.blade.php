                        ...
                        <table class="table table-sm table-hover table-striped tm-table-striped-even">
                            ...
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    <td><a data-backdrop="static" data-keyboard="false" data-toggle="modal" data-target="#update-modal-{{$user->id}}"><i class="fas fa-pencil-alt text-dark"></i></a></td>

                                    <td><a href="{{route('user.delete',$user)}}" onclick="event.preventDefault();document.getElementById('user-delete-{{$user->id}}').submit()"><i class="fas fa-trash-alt text-dark"></i></a></td>
                                    
                                    {{-- HIDDEN FORM FOR DELETE BUTTON --}}
                                    <form id="user-delete-{{$user->id}}" action="{{route('user.delete',$user)}}"
                                        ...
                                        <div class="modal-dialog" role="document">
                                          <div class="modal-content">
                                            <div class="modal-header">
                                              <h5 class="modal-title" id="update-modal-{{$user->id}}-label">{{__('text.EditingUser')}}: {{$user->id}}</h5>
                                              <button type="button" class="close" data-dismiss="modal" aria-label="Close" onclick="document.getElementById('form-{{$user->id}}').reset()">
                                                <span aria-hidden="true">&times;</span>
                                              </button>
                                            </div>
                                        <form action="{{route('user.update',$user)}}" method="POST" id="form-{{$user->id}}">
                                                @csrf
                                                @method('put')
                                                ...
                                                <div class="modal-body">
                                                    ...
                                                    <div class="form-group row">
                                                        <label for="access" class="col-md-4 col-form-label text-md-right">{{ __('text.AdminAccess') }}</label>
                            
                                                        <div class="col-md-6">
                                                            <input type="hidden" name="access" value="0">
                                                            <input id="access" type="checkbox" class="form-control @error('access') is-invalid @enderror" name="access" value="1" {{ $user->access==1 ? 'checked=\'\'' : '' }}" autofocus>
